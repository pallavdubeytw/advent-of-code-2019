...
def get_planets(puzzle_input: str):
...
distinct_planets = get_planets(planet_str)
print(f'total no of planets: {len(distinct_planets)}')


def find_in_tree(root, to_find):
    if root is None:
        return
    ...
    left = find_in_tree(root.left, to_find)
    if left:
        return left

    right = find_in_tree(root.right, to_find)
    if right:
        return right
...
while len(plotted) < len(distinct_planets):
    ...
    for line in planet_str.split(','):
        ...
        if tree is None:
            ...
            if n is not None and n.left is None:
                n.left = Node(p[1])
                n.left.parent = n
                plotted.add(p[1])
                continue
            if n is not None and n.right is None:
                n.right = Node(p[1])
                n.right.parent = n
                plotted.add(p[1])
                continue
...
total = 0
for p in distinct_planets:
    n = find_in_tree(tree, p)
