...
def distinct_planets(input_str: str):
...
def find_node_by_value(root, to_find):
    if root is None:
        return
    ...
    if root.left is not None and root.left.value == to_find:
        return root.left

    if root.right is not None and root.right.value == to_find:
        return root.right

    left = find_node_by_value(root.left, to_find)
    if left:
        return left

    right = find_node_by_value(root.right, to_find)
    if right:
        return right


def generate_tree(input_str, all_planets):
    added_planets = set()
    tree = None
    while len(added_planets) < len(all_planets):
        # print(f'progress: {int(len(added_planets)/len(all_planets)*100)}%')
        for each_pair in input_str.split(','):
            center = each_pair.split(')')[0]
            orbiter = each_pair.split(')')[1]
            center_node = find_node_by_value(tree, center)

            if tree is None:
                tree = Node(center, None)
                tree.left = Node(orbiter, tree)
                added_planets.add(center)
                added_planets.add(orbiter)
            else:
                if center_node is not None and center_node.left is None:
                    center_node.left = Node(orbiter, center_node)
                    added_planets.add(orbiter)
                    continue
                if center_node is not None and center_node.right is None:
                    center_node.right = Node(orbiter, center_node)
                    added_planets.add(orbiter)
                    continue
    return tree


def total_orbit_count(tree, all_planets):
    total = 0
    for planet in all_planets:
        node = find_node_by_value(tree, planet)
        orbit_count = 0
...
def distance_from_root(tree, node_value):
    node = find_node_by_value(tree, node_value)
    node_dist = 0
    while node is not None and node.parent is not None:
        node = node.parent
        node_dist += 1
...
def path_from_root(tree, node_value):
    node = find_node_by_value(tree, node_value)
    node_path = []
    while node is not None and node.parent is not None:
        node = node.parent
...
def distance_btw_two_nodes(tree, node_one_value, node_two_value):
    node_one_distance = distance_from_root(tree, node_one_value)
    node_two_distance = distance_from_root(tree, node_two_value)

    node_one_path = path_from_root(tree, node_one_value)
    node_two_path = path_from_root(tree, node_two_value)

    lca = [planet for planet in node_one_path if planet in node_two_path][0]

    lca_distance = distance_from_root(tree, lca)

    return node_one_distance + node_two_distance - 2 * lca_distance


def main():
    print(f'...')
    all_planets = distinct_planets(puzzle_input)
    tree = generate_tree(puzzle_input, all_planets)
    print(f'total orbits: {total_orbit_count(tree, all_planets)}')
    print(f'distance: {distance_btw_two_nodes(tree, "YOU", "SAN") - 2}')
